"""
GNN-CTDE Training Loop.

One shared GNN encoder + one shared Dueling DQN (central controller).
GNN enriches observations at interaction time; the shared buffer stores
flat enriched vectors — training loop is identical to trainCTDE.py.

Structure mirrors trainGnn.py exactly for a fair comparison.

Usage:
    python trainCtdeGnn.py                        # default config
    python trainCtdeGnn.py --n_ues 10 --n_uavs 2 --episodes 500
"""
...
from ctdeGnnAgent import GNNCTDEAgent
...
def run_episode(
    ...
    At each timestep we need the graph snapshot TWICE:
      - Current  graph → action selection + enriching transitions
      - Next     graph → enriching next-state transitions
    ...
    for _ in range(env.cfg.I):
        ...
        if train:
            ...
            # store() filters per-UE via task_mask internally.
            if any(had_task):
                agent.store(
                    obs_list, graph_data,
                    actions, rewards,
            ...
            # One centralized gradient update per timestep
...
def train(args) -> dict:
    ...
    print(f"\nTraining GNN-CTDE — {args.n_ues} UEs, {args.n_uavs} UAVs, "
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if stats["avg_cost"] < best_cost:
            ...
            if args.save_dir:
                ...
                agent.save(os.path.join(args.save_dir, "gnn_ctde_best.pt"))
    ...
    if args.save_dir:
        os.makedirs(args.save_dir, exist_ok=True)
        agent.save(os.path.join(args.save_dir, "gnn_ctde_final.pt"))
        with open(os.path.join(args.save_dir, "gnn_ctde_results.json"), "w") as f:
...
def get_args():
    ...
    # Environment — keep defaults identical to trainGnn.py for fair comparison
    ...
    # GNN — identical defaults to trainGnn.py
    ...
    # Training — identical defaults to trainGnn.py
...
if __name__ == "__main__":
    args    = get_args()
    results = train(args)
    print(f"\nGNN-CTDE eval  —  {results['eval_mean']:.4f} ± {results['eval_std']:.4f}")