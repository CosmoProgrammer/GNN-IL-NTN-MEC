...
from gnnGatPlusAgent import GATILAgentPlus
...
def train(args) -> dict:
    ...
    print(f"\nTraining GATPlus-IL — {args.n_ues} UEs, {args.n_uavs} UAVs, "
          f"{args.episodes} episodes")
    print(f"GATPlus: hidden={args.gnn_hidden}, out={args.gnn_out}  "
    ...
    for ep in range(1, args.episodes + 1):
        ...
        if stats["avg_cost"] < best_cost:
            ...
            if args.save_dir:
                ...
                agent.save(os.path.join(args.save_dir, "gatplus_il_best.pt"))
    ...
    if args.save_dir:
        os.makedirs(args.save_dir, exist_ok=True)
        agent.save(os.path.join(args.save_dir, "gatplus_il_final.pt"))
        with open(os.path.join(args.save_dir, "gatplus_il_results.json"), "w") as f:
...
def get_args():
    ...
    p.add_argument("--per_beta_steps", type=int, default=None)

    args = p.parse_args()
    if args.per_beta_steps is None:
        args.per_beta_steps = int(args.episodes * args.steps_per_ep * 0.3)
...
if __name__ == "__main__":
    args    = get_args()
    results = train(args)
    print(f"\nGATPlus-IL eval  —  {results['eval_mean']:.4f} ± {results['eval_std']:.4f}")